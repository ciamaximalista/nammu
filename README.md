...
- `core/admin-pending-submission.php`: envío pendiente del editor. Si la sesión de PHP caduca mientras el autor escribe y pulsa Publicar, Guardar como borrador o Actualizar, el texto se conserva en la sesión nueva, la pantalla de acceso lo avisa y, al iniciar sesión, se escribe como borrador y se abre en el editor (nunca se publica; si venía de una entrada existente va a un borrador aparte `<slug>-recuperado.md` y el original no se toca). Mientras el editor está abierto, `autosave.js` hace un latido cada 5 minutos a `admin.php?session_ping=1` para que la sesión no caduque, además de la copia local en `localStorage`.
