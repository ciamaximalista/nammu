...
function nammu_fediverse_fetch_cache_file(): string
{
    return dirname(__DIR__) . '/config/fediverso-fetch-cache.json';
}
...
function nammu_fediverse_cache_control_max_age(array $headers): int
{
    $value = strtolower(trim((string) ($headers['cache-control'] ?? '')));
    if ($value === '') {
        return 0;
    }
    if (preg_match('/(?:^|,)\s*max-age\s*=\s*(\d+)/', $value, $matches) === 1) {
        return max(0, (int) ($matches[1] ?? 0));
    }
    return 0;
}

function nammu_fediverse_fetch_cache_ttl_for_response(int $status, array $headers, string $body = ''): int
{
    $retryAfter = nammu_fediverse_retry_after_seconds($headers);
    if ($retryAfter > 0) {
        return $retryAfter;
    }
    $maxAge = nammu_fediverse_cache_control_max_age($headers);
    if ($maxAge > 0) {
        return $maxAge;
    }
...
function nammu_fediverse_fetch_cache_read(string $url): ?array
{
    $url = trim($url);
    if ($url === '' || !preg_match('#^https?://#i', $url)) {
        return null;
    ...
    $store = nammu_fediverse_fetch_cache_store();
    $entry = $store['items'][nammu_fediverse_fetch_cache_key($url)] ?? null;
    if (!is_array($entry)) {
        return null;
    }
...
function nammu_fediverse_fetch_cache_conditional_headers(?array $entry): array
{
    if (!is_array($entry) || (int) ($entry['status'] ?? 0) < 200 || (int) ($entry['status'] ?? 0) >= 300) {
        return [];
    }
    ...
    if ($lastModified !== '') {
        $result['If-Modified-Since'] = $lastModified;
    }
    return $result;
}
...
function nammu_fediverse_fetch_cache_refresh_not_modified(string $url, array $responseHeaders = []): ?array
{
    ...
    $headers = array_merge(is_array($entry['headers'] ?? null) ? $entry['headers'] : [], $responseHeaders);
    $ttl = nammu_fediverse_fetch_cache_ttl_for_response((int) ($entry['status'] ?? 200), $headers, (string) ($entry['body'] ?? ''));
    if ($ttl <= 0) {
        $ttl = 300;
    }
...
function nammu_fediverse_fetch_cache_write(string $url, array $response): void
{
    $url = trim($url);
    if ($url === '' || !preg_match('#^https?://#i', $url)) {
        return;
    ...
    $ttl = nammu_fediverse_fetch_cache_ttl_for_response($status, $headers, $body);
    if ($ttl <= 0) {
        return;
    }
    $key = nammu_fediverse_fetch_cache_key($url);
    ...
    if (($status >= 500 || $status === 0) && $failureCount > 1) {
        $ttl = min(3600, max($ttl, (int) (120 * (2 ** min(4, $failureCount - 1))) + random_int(0, 60)));
    }
    $entry = [
        'url' => $url,
        'status' => $status,
        'headers' => array_intersect_key($headers, array_flip(['cache-control', 'etag', 'last-modified', 'retry-after', 'content-type'])),
        'body' => ($status >= 200 && $status < 300) ? $body : '',
        'fetched_at' => time(),
        'expires_at' => time() + $ttl,
        'failure_count' => $failureCount,
    ];
...
function nammu_fediverse_signed_fetch(string $url, array $config, string $method = 'GET', string $body = '', ?int $timeoutOverride = null, array $extraHeaders = []): array
{
    ...
    $headers = nammu_fediverse_signature_header($method, $url, $config, $body, $extraHeaders) ?? [
        'User-Agent: Nammu Fediverso',
        'Accept: application/activity+json, application/ld+json; profile="https://www.w3.org/ns/activitystreams", application/json;q=0.9',
    ];
    if (!empty($extraHeaders)) {
        foreach ($extraHeaders as $name => $value) {
            $headerLine = trim((string) $name) . ': ' . trim((string) $value);
            if (!in_array($headerLine, $headers, true)) {
                $headers[] = $headerLine;
            }
        }
    }
...
function nammu_fediverse_signed_fetch_json(string $url, array $config, string $method = 'GET', string $body = '', ?int $timeoutOverride = null): ?array
{
    ...
    $method = strtoupper($method);
    $cacheKey = $method . '|' . $url;
    if ($method === 'GET' && $body === '' && array_key_exists($cacheKey, $requestCache)) {
        return is_array($requestCache[$cacheKey]) ? $requestCache[$cacheKey] : null;
    }
    if ($method === 'GET' && $body === '') {
        $cachedFetch = nammu_fediverse_fetch_cache_read($url);
        if (is_array($cachedFetch)) {
            $cachedStatus = (int) ($cachedFetch['status'] ?? 0);
            if ($cachedStatus >= 200 && $cachedStatus < 300) {
                $decoded = json_decode((string) ($cachedFetch['body'] ?? ''), true);
                $requestCache[$cacheKey] = is_array($decoded) ? $decoded : null;
                return is_array($decoded) ? $decoded : null;
            }
    ...
    $staleFetch = ($method === 'GET' && $body === '') ? nammu_fediverse_fetch_cache_peek($url) : null;
    $conditionalHeaders = nammu_fediverse_fetch_cache_conditional_headers($staleFetch);
    if (
        $method === 'GET'
        && $body === ''
    ...
    if ($method === 'GET' && $body === '' && (int) ($response['status'] ?? 0) === 304) {
        ...
        if (is_array($refreshed)) {
            $decoded = json_decode((string) ($refreshed['body'] ?? ''), true);
            $requestCache[$cacheKey] = is_array($decoded) ? $decoded : null;
            return is_array($decoded) ? $decoded : null;
        }
    }
...
function nammu_fediverse_fetch(string $url, string $accept = 'application/activity+json, application/ld+json; profile="https://www.w3.org/ns/activitystreams", application/json;q=0.9', int $timeout = 12, array $extraHeaders = []): array
{
    $headers = [];
    $headerLines = [
        'User-Agent: Nammu Fediverso',
...
function nammu_fediverse_warm_threads_cache(array $config, int $limit = 20): int
{
    ...
    foreach ($timelineItems as $item) {
        ...
        $type = strtolower(trim((string) ($item['type'] ?? '')));
        if (in_array($type, ['like', 'delete', 'announce'], true)) {
            continue;
        }
        $objectId = trim((string) (($item['object_id'] ?? '') ?: ($item['id'] ?? '')));
        ...
        if (!array_key_exists($objectId, $cacheItems)) {
            continue;
        }
...
function nammu_fediverse_build_home_thread_payloads(array $localItems, array $config, array $reactionSummary, array $reactionDetails, array $incomingReplies, array $existingThreadPayloads = []): array
{
    ...
    $primaryItemsByUrl = [];
    $identifiersByUrl = [];
    foreach ($localItems as $candidateItem) {
        if (!is_array($candidateItem)) {
            continue;
        }
        $candidateUrl = trim((string) ($candidateItem['url'] ?? ''));
        $candidateId = trim((string) ($candidateItem['id'] ?? ''));
        if ($candidateUrl !== '') {
            if (
                !isset($primaryItemsByUrl[$candidateUrl])
                && $candidateId !== ''
            ) {
                $primaryItemsByUrl[$candidateUrl] = $candidateItem;
            }
            if (!isset($identifiersByUrl[$candidateUrl])) {
                $identifiersByUrl[$candidateUrl] = [];
            }
            foreach (['id', 'url'] as $field) {
                $identifier = trim((string) ($candidateItem[$field] ?? ''));
                if ($identifier !== '') {
                    $identifiersByUrl[$candidateUrl][$identifier] = true;
                }
            }
        }
    }
    foreach ($localItems as $localItem) {
        ...
        $localUrl = trim((string) ($localItem['url'] ?? ''));
        $canonicalItem = ($localUrl !== '' && is_array($primaryItemsByUrl[$localUrl] ?? null))
            ? $primaryItemsByUrl[$localUrl]
            : $localItem;
        ...
        $targetIdentifiers = [];
        foreach (['id', 'url'] as $field) {
            $identifier = trim((string) ($canonicalItem[$field] ?? ''));
            if ($identifier !== '') {
                $targetIdentifiers[$identifier] = true;
            }
        }
        if ($localUrl !== '' && isset($identifiersByUrl[$localUrl])) {
            foreach (array_keys($identifiersByUrl[$localUrl]) as $identifier) {
                $targetIdentifiers[$identifier] = true;
            }
        }
        $targetIdentifiers = array_keys($targetIdentifiers);
...
function nammu_fediverse_cli_timing_log(string $scope, string $step, float $startedAt, array $extra = []): void
{
    if (PHP_SAPI !== 'cli') {
        return;
    }
    if (function_exists('admin_cli_timing_log')) {
        admin_cli_timing_log($scope, $step, $startedAt, $extra);
        return;
    }
    $env = getenv('NAMMU_DEBUG');
    $debugEnabled = false;
    if ($env !== false) {
        $debugEnabled = in_array(strtolower(trim((string) $env)), ['1', 'true', 'on', 'yes'], true);
    } elseif (isset($GLOBALS['argv']) && is_array($GLOBALS['argv'])) {
        $debugEnabled = in_array('--debug', $GLOBALS['argv'], true);
    }
    if (!$debugEnabled) {
        return;
    }
...
function nammu_fediverse_rebuild_home_snapshot(array $config): array
{
    ...
    nammu_fediverse_cli_timing_log($scope, 'known_actors', $stepStartedAt, [
        'count' => count($actorsById),
    ]);
    $stepStartedAt = microtime(true);
    $localItems = nammu_fediverse_local_content_items($config);
    nammu_fediverse_cli_timing_log($scope, 'local_content_items', $stepStartedAt, [
    ...
    nammu_fediverse_cli_timing_log($scope, 'merge_resend_items', $stepStartedAt, [
        'count' => count($localItems),
    ]);
    $stepStartedAt = microtime(true);
    $localReactionSummary = nammu_fediverse_local_reaction_summary($config);
    nammu_fediverse_cli_timing_log($scope, 'local_reaction_summary', $stepStartedAt, [
        'count' => count($localReactionSummary),
    ]);
    $stepStartedAt = microtime(true);
    $localReactionDetails = nammu_fediverse_local_reaction_details($config);
    nammu_fediverse_cli_timing_log($scope, 'local_reaction_details', $stepStartedAt, [
        'count' => count($localReactionDetails),
    ]);
    $stepStartedAt = microtime(true);
    $incomingReplies = nammu_fediverse_incoming_public_replies_by_object($config);
    nammu_fediverse_cli_timing_log($scope, 'incoming_public_replies', $stepStartedAt, [
        'targets' => count($incomingReplies),
    ]);
    $data = [
    ...
    nammu_fediverse_cli_timing_log($scope, 'build_home_thread_payloads', $stepStartedAt, [
        'count' => count($data['thread_payloads']),
    ]);
    $stepStartedAt = microtime(true);
    nammu_fediverse_promote_thread_payloads_to_state_store($data['thread_payloads'], $config);
    nammu_fediverse_cli_timing_log($scope, 'promote_thread_payloads_to_state_store', $stepStartedAt, [
        'count' => count($data['thread_payloads']),
    ]);
    $stepStartedAt = microtime(true);
    nammu_fediverse_promote_thread_payloads_to_threads_cache($data['thread_payloads'], $config);
    nammu_fediverse_cli_timing_log($scope, 'promote_thread_payloads_to_threads_cache', $stepStartedAt, [
        'count' => count($data['thread_payloads']),
    ]);
    $stepStartedAt = microtime(true);
    nammu_fediverse_save_home_snapshot_store($data);
    nammu_fediverse_cli_timing_log($scope, 'save_home_snapshot_store', $stepStartedAt);
    return $data;
}
...
function nammu_fediverse_rebuild_notifications_snapshot(array $config): array
{
    ...
    nammu_fediverse_cli_timing_log($scope, 'notification_entries', $stepStartedAt, [
        'count' => count((array) ($data['notifications'] ?? [])),
    ]);
    $stepStartedAt = microtime(true);
    nammu_fediverse_save_notifications_snapshot_store($data);
    nammu_fediverse_cli_timing_log($scope, 'save_notifications_snapshot_store', $stepStartedAt);
    return $data;
}
...
function nammu_fediverse_rebuild_light_snapshots(array $config): array
{
    ...
    $stepStartedAt = microtime(true);
    nammu_fediverse_refresh_legacy_actuality_aliases($config);
    nammu_fediverse_cli_timing_log($scope, 'refresh_legacy_actuality_aliases', $stepStartedAt);
    $stepStartedAt = microtime(true);
    $home = nammu_fediverse_rebuild_home_snapshot($config);
...
function nammu_fediverse_normalize_remote_item(array $activity, array $actor, array $config): ?array
{
    ...
    $scope = 'normalize_remote_item';
    $itemLabel = $activityId !== '' ? $activityId : trim((string) (($activity['url'] ?? '') ?: ($activity['object']['id'] ?? '')));
    if ($type === 'like') {
        return null;
    }
    $object = $activity;
    ...
    if ($type === 'announce') {
        ...
        if (is_string($announcedObject)) {
            ...
            if (preg_match('#^/ap/objects/(actualidad|post|podcast|itinerary)-[^/]+$#', $objectPath) === 1) {
                ...
                $object = [
                    'id' => $objectId,
                    'url' => nammu_fediverse_remote_thread_page_url($objectId),
                    'name' => $fallbackTitle,
                    'summary' => 'Impulsó una publicación.',
                ];
            } else {
...
{
    $actorId = trim((string) ($actor['id'] ?? ''));
    $actorLabel = trim((string) (($actor['name'] ?? '') ?: ($actor['preferredUsername'] ?? '') ?: $actorId));
    $rawItems = [];
    $items = [];
    $topLevelItems = [];
    $otherItems = [];
...
function nammu_fediverse_sync_recent_followed_inbox_items(array $config, int $limit = 6, int $scanLimit = 60): array
{
    ...
    $activityCandidateIds = static function (array $payload): array {
        ...
        $object = $payload['object'] ?? null;
        if (is_string($object)) {
            $objectId = trim($object);
            if ($objectId !== '') {
                $ids[$objectId] = true;
            }
        } elseif (is_array($object)) {
            foreach (['id', 'url'] as $field) {
                $objectId = nammu_fediverse_extract_url($object[$field] ?? '');
                if ($objectId === '') {
                    $objectId = trim((string) ($object[$field] ?? ''));
                }
                if ($objectId !== '') {
                    $ids[$objectId] = true;
                }
    ...
    foreach ($activities as $entry) {
        ...
        foreach ($candidateIds as $candidateId) {
            if (isset($knownTimelineIds[$candidateId])) {
                $alreadyKnown = true;
                break;
            }
        }
...
function nammu_fediverse_incoming_public_replies_by_object(array $config): array
{
    ...
    $hiddenLookup = nammu_fediverse_hidden_reply_lookup();
    $knownActorsById = [];
    foreach (nammu_fediverse_known_actors() as $knownActor) {
        if (!is_array($knownActor)) {
            continue;
        }
        $knownActorId = trim((string) ($knownActor['id'] ?? ''));
        if ($knownActorId !== '') {
            $knownActorsById[$knownActorId] = $knownActor;
        }
    }
    usort($activities, static function (array $a, array $b): int {
...
{
    $itemId = trim((string) ($matchedItem['id'] ?? ''));
    if ($itemId === '') {
        return ['ok' => false, 'message' => 'No se pudo identificar el objeto a reenviar.'];
    }
    $followers = nammu_fediverse_followers_store()['followers'];
    if (empty($followers)) {
    ...
    $activity['id'] = $itemId . '/activity-resend-' . substr(sha1($resendKey . '|' . $resendPublished), 0, 16);
...
function nammu_fediverse_deliver_named_local_item(string $slug, string $template, array $config): array
{
    ...
    $supportedTemplates = ['post', 'page', 'podcast', 'itinerary'];
    if (!in_array($template, $supportedTemplates, true)) {
        return ['ok' => false, 'message' => 'Solo las entradas, páginas, podcasts e itinerarios pueden reenviarse al Fediverso desde Editar.'];
    }
    ...
    if (!is_array($matchedItem)) {
        ...
        if (is_file($contentFile) && is_readable($contentFile)) {
            ...
            if (
                $status === 'published'
                && $visibility !== 'private'
                && in_array($normalizedTemplate, ['post', 'page', 'podcast'], true)
    ...
    return nammu_fediverse_resend_local_item_to_followers($matchedItem, $config, $slug . '|' . $template);
}

function nammu_fediverse_deliver_actuality_item(string $id, array $config): array
{
    $id = trim($id);
    if ($id === '') {
        return ['ok' => false, 'message' => 'No se pudo identificar el contenido de actualidad a reenviar.'];
    }
    $baseUrl = rtrim(nammu_fediverse_base_url($config), '/');
    ...
    if (!is_array($matchedItem)) {
        return ['ok' => false, 'message' => 'No se encontró ese contenido de actualidad publicado para enviarlo al Fediverso.'];
    }
    return nammu_fediverse_resend_local_item_to_followers($matchedItem, $config, 'actuality|' . $id);
}
...
function nammu_fediverse_public_thread_meta_for_named_local_item(string $slug, string $template, array $config): array
{
    ...
    if (!is_array($snapshotMeta) && is_array($snapshotKeyMeta)) {
        $snapshotMeta = $snapshotKeyMeta;
    }
    ...
    if (is_array($item)) {
        ...
        if (
            is_array($bestMeta)
            && (
                !is_array($snapshotMeta)
                || nammu_fediverse_thread_meta_score($bestMeta) > nammu_fediverse_thread_meta_score($snapshotMeta)
            )
        ) {
            $snapshotMeta = $bestMeta;
        }
    }
    ...
    if (is_array($snapshotMeta)) {
        $snapshotPayload = [
            'item' => is_array($item) ? $item : ['id' => $candidateId, 'url' => $candidateUrl],
            'thread_url' => trim((string) ($snapshotMeta['thread_url'] ?? $threadUrl)),
            'original_url' => $candidateUrl,
            'summary' => is_array($snapshotMeta['summary'] ?? null) ? $snapshotMeta['summary'] : ['likes' => 0, 'shares' => 0, 'replies' => 0],
            'details' => is_array($snapshotMeta['details'] ?? null) ? $snapshotMeta['details'] : ['likes' => [], 'shares' => [], 'replies' => []],
            'replies' => [],
        ];
        $payload = is_array($payload)
            ? nammu_fediverse_merge_thread_payload_metrics($payload, $snapshotPayload)
            : $snapshotPayload;
    }
