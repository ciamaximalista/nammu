...
$renderHeaderButtons = static function () use ($homeUrl, $searchAction, $showLetterButton, $letterIndexUrlValue, $hasCategories, $categoriesIndexUrl, $hasItineraries, $itinerariesIndexUrl, $hasPodcast, $podcastIndexUrl, $hasNewsletters, $newslettersIndexUrl, $subscriptionEnabled, $subscriptionAction, $postalEnabled, $postalUrl, $postalLogoSvg, $colorAccent, $colorHighlight): string {
    $items = [];
    $items[] = [
        'label' => 'Portada',
        'href' => $homeUrl,
    ...
    <div class="post-header-buttons">
        <?php foreach ($items as $item): ?>
            <a class="post-header-button-link" href="<?= htmlspecialchars($item['href'], ENT_QUOTES, 'UTF-8') ?>" title="<?= htmlspecialchars($item['label'], ENT_QUOTES, 'UTF-8') ?>" aria-label="<?= htmlspecialchars($item['label'], ENT_QUOTES, 'UTF-8') ?>">
                <?= $item['svg'] ?>
            </a>
        <?php endforeach; ?>
    </div>
